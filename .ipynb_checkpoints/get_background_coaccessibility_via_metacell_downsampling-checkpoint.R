...
parser$add_argument("--norm_atac_file",
                   help = "matrix of normalized ATAC, rows = peaks, columns = cells/metacells (supply this or archr_cell_type")
...
parser$add_argument("--num_peak_pairs", default = 100000,
                   help = "number of peak pairs across which to commpute average co-accessibility")
...
num_peak_pairs = as.numeric(args$num_peak_pairs)
...
set.seed(seed)
peak1 = sample(peaks, num_peak_pairs, replace = TRUE)
peak2 = rep(NA, num_peak_pairs)
for (i in 1:length(peak1)) {
    focal_peak = peak1[i]
    focal_chrom = peak_chrom[[focal_peak]]
    peak2[i] = sample(names(peak_chrom[peak_chrom != focal_chrom]), 1)
}

# Compute peak-peak squared correlations with original and downsampled matrix
coaccessibility_sq_norm = rep(NA, num_peak_pairs)
coaccessibility_sq_downsampled = rep(NA, num_peak_pairs)
for (i in 1:num_peak_pairs) {
    coaccessibility_sq_norm[i] = cor(norm_atac[peak1[i],], norm_atac[peak2[i],]) ** 2
    coaccessibility_sq_downsampled[i] = cor(downsampled_atac[peak1[i],], downsampled_atac[peak2[i],]) ** 2
    print(i)
}
...
# Compute averages and average difference
